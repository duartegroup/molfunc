...
namespace molfunc{
    ...
    void RotationMatrix::update(double w1, double w2, double w3) {
        ...
        __elems_[0][0] = 1 - b * (w3 * w3 + w2 * w2);
        __elems_[0][1] = - a * w3 + b * w2 * w1;
        __elems_[0][2] = a * w2 + b * w3 * w1;

        // Second row of the matrix
        __elems_[1][0] = a * w3 + b * w1 * w2;
        __elems_[1][1] = 1 - b * (w3 * w3 + w1 * w1);
        __elems_[1][2] = - a * w1 + b * w2 * w3;

        // Thrid row of the matrix
        __elems_[2][0] = -a * w2 + b * w1 * w3;
        __elems_[2][1] = a * w1 + b * w2 * w3;
        __elems_[2][2] = 1 - b * (w1 * w1 + w2 * w2);
    }
}
